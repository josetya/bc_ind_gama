...
class MyToggleButtons<T> extends StatelessWidget {
  ...
  BorderSide _getLeadingBorderSide(
    ...
    final double resultingBorderWidth =
        borderWidth ?? toggleButtonsTheme.borderWidth ?? _defaultBorderWidth;
    if (onPressed != null &&
        (_isSelected(index) || (index != 0 && _isSelected(index - 1)))) {
      return BorderSide(
      ...
      children: items
          ...
          .map((int itemIndex, T item) {
            final BorderRadius edgeBorderRadius = _getEdgeBorderRadius(
                itemIndex, items.length, textDirection, toggleButtonsTheme);
            final BorderRadius clipBorderRadius = _getClipBorderRadius(
                itemIndex, items.length, textDirection, toggleButtonsTheme);

            final BorderSide leadingBorderSide =
                _getLeadingBorderSide(itemIndex, theme, toggleButtonsTheme);
            final BorderSide horizontalBorderSide =
                _getHorizontalBorderSide(itemIndex, theme, toggleButtonsTheme);
            final BorderSide trailingBorderSide =
                _getTrailingBorderSide(itemIndex, theme, toggleButtonsTheme);
            ...
            return MapEntry(
              ...
              Expanded(
                ...
                onPressed: onPressed != null
                    ...
                    : null,
                isFirstButton:
                    _isFirstIndex(itemIndex, items.length, textDirection),
                isLastButton:
                    _isLastIndex(itemIndex, items.length, textDirection),
                child: _isSelected(itemIndex)
      ...
      case TextDirection.ltr:
        if (isLastButton) {
          ...
          final Path endingPath = Path()
            ...
            ..lineTo(
                rrect.left + horizontalBorderSide.width / 2.0, rrect.bottom);
          ...
          final Path horizontalPaths = Path()
            ...
            ..moveTo(
                rrect.left + horizontalBorderSide.width / 2.0 + rrect.tlRadiusX,
                rrect.bottom)
      ...
      case TextDirection.rtl:
        if (isLastButton) {
          ...
          final Path endingPath = Path()
            ...
            ..lineTo(
                rrect.right - horizontalBorderSide.width / 2.0, rrect.bottom);
          context.canvas.drawPath(endingPath, endingPaint);
        } else if (isFirstButton) {
          final Path leadingPath = Path()
